...
def plot_pos_prob_prev(
    ...
    fig.add_scatter(
        ...
        y=df["tests_pos"].values/df["tests"].values,
...
def plot_pos_prob_prev_plt(
    ...
    Prev_post.index = pd.to_datetime(Prev_post.index)
    
    fig, ax = plt.subplots(dpi = 300)
    
    ax.plot(df["tests_pos"].index, df["tests_pos"].values/df["tests"].values, '.', color = 'tab:purple', markersize = 4)
    
    Prev_med = Prev_post.rolling(7).mean().dropna()
    
    ax.plot(Prev_med.upper, color = 'tab:green', lw =1)
    
    ax.plot(Prev_med['median'], color = 'tab:blue', lw =1)
    
    ax.plot(Prev_med.lower, color = 'tab:red', lw =1)
    
    ax.set_xlabel('date')
    ax.set_ylabel('prevalence of infected')
    ...
    plt.savefig('plots/prevalence_est_plt.png', dpi = 300)
    

    return


def plot_pos_prob_hosp_plt(
    df: pd.DataFrame,
    tracevb: az.data.inference_data.InferenceData,
):
    """
    ...
    Phost_med = Phosp_post.rolling(7).mean().dropna()
    
    ax.plot(Phost_med.upper, color = 'tab:green', lw =1)
    
    ax.plot(Phost_med['median'], color = 'tab:blue', lw =1)
    
    ax.plot(Phost_med.lower, color = 'tab:red', lw =1)
    ...
    plt.savefig("plots/prob_hosp_est_plt.png", dpi = 300)
